    ...
    def forward(self):
        if self.type_node == 'Load':
            ...
            if type_surface == 'Sphere':
                ...
                load_data = scio.loadmat('./SUOT-ADMM-RBF/Data_Set/surface/' + file_name + '.mat')
                ...
                load_data = trimesh.load('./SUOT-ADMM-RBF/Data_Set/surface/' + file_name + '.obj')
...
class Train_loop(object):
    ...
    def train(self):
        ...
        def RBF_Tangent_Plane_Poly(self, nodes, elements):
            ...
            def Projection_nodes(nodes_center, normal_center, nodes_local, normal_local):
                nodes_local_pro = np.zeros(nodes_local.shape)
                for i in range(nodes_local.shape[0]):
                    temp1 = np.dot(normal_local[i, :], normal_center)
                    temp2 = np.dot(nodes_center - nodes_local[i, :], normal_center)
                    nodes_local_pro[i, :] = nodes_local[i, :] + temp2 / temp1 * normal_local[i, :]
                
                return nodes_local_pro
            ...
            # for i in range(M):
                ...
                normal_local = normal[id_y, :]
                new_nodes_local = Projection_nodes(nodes_center, nodes_normal, nodes_local, normal_local)
                new_nodes_local = np.dot(new_nodes_local, R.T)[:, 0:2]
                # new_nodes_local1 = np.dot(nodes_local, R.T)[:, 0:2]
        ...
        if self.Key_Para['type_surface'] == 'Sphere':
            ...
            cur_rho_bar[:, 0] = Sphere_Surface_Gaussian(self, nodes, self.Key_Para['mu_0'], self.Key_Para['sigma_0']).reshape(-1) + \
                                Sphere_Surface_Gaussian(self, nodes, np.array(nodes[5886, :]).reshape(1, -1), self.Key_Para['sigma_0']).reshape(-1)
            cur_rho_bar[:, -1] = 2*Sphere_Surface_Gaussian(self, nodes, self.Key_Para['mu_1'], self.Key_Para['sigma_1']).reshape(-1)
        else:
            cur_rho_bar[:, 0] = Sphere_Surface_Gaussian(self, nodes, self.Key_Para['mu_0'], self.Key_Para['sigma_0']).reshape(-1)
            cur_rho_bar[:, -1] = Sphere_Surface_Gaussian(self, nodes, self.Key_Para['mu_1'], self.Key_Para['sigma_1']).reshape(-1)
